...
if (!is_null($resumen['tributos'])) {
  ...
  foreach ($tributos as $index => $tributo) {
      ...
      $pdf->SetXY( 148, $y_detalle); //posicion de la celda
      $pdf->SetFont('helvetica', '', $texto); //formato de la celda
      $pdf->Cell(0, 3, numer_format($tributos_descrip,2,'.',',') , 0, 1, 'L');//contenido de la celda
...
if($print == '1'){
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = $nombre;
        $mail->Body    = '<b>Buen dia, '.$nombreReceptor.'</b><br>Adjunto encontraras el documento PDF de tu factura y el archivo Json.';
